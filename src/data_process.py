...
class DataProcess(object):
    ...
    def load_node(self):
        ...
        with open(self.input_edge) as file:
            ...
            for line in file:
                ...
                # source_type == data_type[source_id] ???
    ...
    def load_matrix(self):
        ...
        with open(self.input_edge) as file:
            ...
            for i in link_type:
                ...
                self.adj_matrix[i] = np.zeros(
                    [source_num, target_num], dtype=float)
            for line in file:
                token = line.strip('\n').split('\t')
                st, tt = token[2].split('-')
    ...
    def generate_matrix(self, combination):
        ...
        for c in combination:
            ...
            self.adj_matrix[target] = np.matmul(
                np.transpose(
                    self.adj_matrix[source1]),
                self.adj_matrix[source2])
        return self.adj_matrix
